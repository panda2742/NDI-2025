...
import { openApp, closeApp, minimizeApp } from "#/api/appController.ts";
...
export const Terminal = () => {
    ...
    const C = ({
        ...
        b = false,
        children,
    }: PropsWithChildren<ColorInterface>) => {
    ...
    const CommandOutput = ({ children }: PropsWithChildren) => {
        return (
            <div className="line command-out" key={`out-${lineNumber}`}>
                {children}
            </div>
        );
    };
    ...
    const newLine = () => {
        if (lastInput.current) lastInput.current.readOnly = true;
        ...
        addTerminalContent(
            <Line gap={"8px"}>
                <C c="green">
                    ...
                    >
                        <path
                            d="M1.5 4H9.5M9.5 4L6.5 1M9.5 4L6.5 7"
                            strokeWidth="2"
                            strokeLinecap="round"
                        />
                    </svg>
    ...
    const commandsList: commandsList = [
        ...
        },
        {
            ...
            commandUsage: "cd <dir>",
            commandDesc:
                'Move into <dir>, "cd .." to go back, "cd" to go to home',
            cb: (args) => {
                ...
                if (!node) {
                    return (
                        <C c="red">cd: no such file or directory: {args[0]}</C>
                    );
                }
        ...
        },
        {
            ...
            cb: (args) => {
                ...
                if (!node) {
                    return (
                        <C c="red">cat: {args[0]}: No such file or directory</C>
                    );
                }
                ...
                return (
                    <div className="list">
                        {commandsList.map((command, i) => (
                            <Li key={i}>
                                <C c="red">{command.commandUsage}</C>{" "}
                                <C c="white"> - {command.commandDesc}</C>
                            </Li>
                        ))}
                    </div>
                );
        ...
        {
            commandName: "open",
            commandUsage: "open <app-id>",
            commandDesc: "Open an application by its ID",
            cb: (args) => {
                if (args.length === 0) {
                    return <C c="red">open: missing app ID</C>;
                }
                ...
                if (success) {
                    return <C c="green">Opened app: {appId}</C>;
                } else {
                    return <C c="red">Failed to open app: {appId}</C>;
                }
            },
        },
        {
            commandName: "kill",
            commandUsage: "kill <app-id>",
            commandDesc: "Close an application by its ID",
            cb: (args) => {
                if (args.length === 0) {
                    return <C c="red">kill: missing app ID</C>;
                }

                const appId = args[0];
                const success = closeApp(appId);

                if (success) {
                    return <C c="green">✓ Closed app: {appId}</C>;
                } else {
                    return <C c="red">✗ Failed to close app: {appId}</C>;
                }
            },
        },
        {
            commandName: "minimize",
            commandUsage: "minimize <app-id>",
            commandDesc: "Minimize an application by its ID",
            cb: (args) => {
                if (args.length === 0) {
                    return <C c="red">minimize: missing app ID</C>;
                }

                const appId = args[0];
                const success = minimizeApp(appId);

                if (success) {
                    return <C c="green">✓ Minimized app: {appId}</C>;
                } else {
                    return <C c="red">✗ Failed to minimize app: {appId}</C>;
                }
            },
        },
        {
            commandName: "apps",
            commandUsage: "apps",
            commandDesc: "List all available application IDs",
            cb: () => {
                const pinnedApps = [
                    "whatsweb",
                    "server",
    ...
    const commandManager = (value: string | undefined) => {
        ...
        if (!command) {
            addTerminalContent(
                <CommandOutput>
                    <C c="red">zsh: command not found: {commandName}</C>
                </CommandOutput>,
            );
            return;
        }
    ...
    return (
        <div
            className="terminal terminal-app"
            id="terminal"
            onClick={handleClick}
        >
            <div
