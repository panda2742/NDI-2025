...
export const App = ({
    ...
    // contraintes max supplémentaires
    const MAX_WIDTH = 2350;   // ajuste si besoin
    const MAX_HEIGHT = 1200;   // ajuste si besoin

    const [dimensions, setDimensions] = useState({width: 777, height: 430});
    const [position, setPosition] = useState({left: 100, top: 100});
    const [isResizing, setIsResizing] = useState(false);
    // ajouté 'bottom' pour resize depuis le bas
    const [resizeDir, setResizeDir] = useState<'left' | 'right' | 'corner' | 'bottom' | null>(null);
    const startPosRef = useRef({x: 0, y: 0});
    const startRectRef = useRef<DOMRect | null>(null);
    const containerRectRef = useRef<DOMRect | null>(null);

    // initialise position/dimensions à partir du DOM si possible
    useEffect(() => {
        const el = component.current;
        if (!el) return;
        const rect = el.getBoundingClientRect();
        startRectRef.current = rect;
        // clamp initial size/position to container if present
        const container = document.querySelector('.apps-container') as HTMLElement | null;
        ...
        if (containerRect) {
            ...
            // applique aussi les limites MAX_WIDTH / MAX_HEIGHT
            setDimensions({
                width: Math.max(MIN_WIDTH, Math.min(rect.width, maxW, MAX_WIDTH)),
                height: Math.max(MIN_HEIGHT, Math.min(rect.height, maxH, MAX_HEIGHT))
            });
            const clampedLeft = Math.max(Math.round(containerRect.left), Math.round(rect.left));
            const clampedTop = Math.max(Math.round(containerRect.top), Math.round(rect.top));
    ...
    const handleResizeStart = (e: React.MouseEvent, dir: 'left' | 'right' | 'corner' | 'bottom') => {
        e.preventDefault();
        e.stopPropagation();
        if (!component.current) return;
        // capture container rect at start of drag
        const container = document.querySelector('.apps-container') as HTMLElement | null;
        ...
        // capture aussi le rect de départ pour calculs
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            // helpers to fall back to window bounds if no container
            ...
            if (resizeDir === 'right') {
                // applique max venant du container et le MAX_WIDTH global
                const maxWFromContainer = Math.round(containerRight - startRect.left);
                const allowedMaxW = Math.min(MAX_WIDTH, maxWFromContainer);
                const proposedW = Math.round(startRect.width + dx);
                const newW = Math.max(MIN_WIDTH, Math.min(proposedW, allowedMaxW));
                setDimensions(prev => ({...prev, width: newW}));
            } else if (resizeDir === 'bottom') {
                // resize depuis le bord inférieur (seul height change)
                const maxHFromContainer = Math.round(containerBottom - startRect.top);
                const allowedMaxH = Math.min(MAX_HEIGHT, maxHFromContainer);
                const proposedH = Math.round(startRect.height + dy);
                const newH = Math.max(MIN_HEIGHT, Math.min(proposedH, allowedMaxH));
                setDimensions(prev => ({...prev, height: newH}));
            } else if (resizeDir === 'left') {
                // dragging left edge: width grows/shrinks and left moves, but not outside container
                const maxWFromContainerLeft = Math.round(startRect.right - containerLeft); // max width when left edge hits containerLeft
                const allowedMaxWLeft = Math.min(MAX_WIDTH, maxWFromContainerLeft);
                const proposedW = Math.round(startRect.width - dx);
                const newW = Math.max(MIN_WIDTH, Math.min(proposedW, allowedMaxWLeft));
                const newLeft = Math.round(startRect.right - newW);
                setDimensions(prev => ({...prev, width: newW}));
                setPosition(prev => ({...prev, left: Math.max(newLeft, Math.round(containerLeft))}));
            } else if (resizeDir === 'corner') {
                const maxWFromContainer = Math.round(containerRight - startRect.left);
                const maxHFromContainer = Math.round(containerBottom - startRect.top);
                const allowedMaxW = Math.min(MAX_WIDTH, maxWFromContainer);
                const allowedMaxH = Math.min(MAX_HEIGHT, maxHFromContainer);
                const newW = Math.max(MIN_WIDTH, Math.min(Math.round(startRect.width + dx), allowedMaxW));
                const newH = Math.max(MIN_HEIGHT, Math.min(Math.round(startRect.height + dy), allowedMaxH));
    ...
    return <div
        ...
        style={{position: 'absolute', left: `${position.left}px`, top: `${position.top}px`, width: `${dimensions.width}px`, height: `${dimensions.height}px`}}
    >
        ...
        {/* handles: left, right, bottom-right corner */}
        <div
